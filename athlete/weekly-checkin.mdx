...
description: "Submit your weekly check-in in FocusFlow: rate energy, sleep, stress, and diet adherence, enter body measurements, and upload Front, Side, and Back progress photos."
---
...
<Warning>
  Progress photos are stored as base64 data URLs inside the `Review` document and in your browser's `localStorage`. They are **not uploaded to Firebase Firestore** due to document size constraints. This means photos are **device-specific** — if you clear your browser storage or switch to a different device, previously saved photos will no longer be visible in the Gallery tab. Use the **Google Drive sync** option (toggle the "Sync to Drive" switch before submitting) to permanently back up your photos to the cloud.
</Warning>
...
interface Review {
  ...
  status: "pending";            // always "pending" on initial submission
  acknowledgedByClient: false;  // cleared once you read coach feedback
