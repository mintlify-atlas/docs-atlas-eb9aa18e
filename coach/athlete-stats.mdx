...
<Card title="Percentage Indicator" icon="percent">
  A single-figure compliance metric sourced from `WorkoutSummary.completionRate` and stored on `ClientProfile.routineComplianceRate`. Represents the ratio of completed sets (achieved) to the total planned sets (`setsCount × routineExercises`) across the athlete's logged history.
</Card>
...
| Compliance Rate | `WorkoutSummary.completionRate` | `fit_summaries` |
...
<CardGroup cols={2}>
  ...
  <Card title="Hover Tooltips" icon="cursor-arrow-rays">
    `<Tooltip>` with a dark theme (`backgroundColor: "#020617"`) is enabled on all charts. Hovering any data point reveals the exact value and date.
  </Card>
