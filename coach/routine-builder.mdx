...
<Steps>
  ...
  <Step title="Add exercises to each day">
    ...
    Click any result to add it instantly. If the exercise doesn't exist in the catalog yet, type its name and click **Crear y Añadir** to create it on the fly and save it to your catalog simultaneously.
...
By default, all sets in an exercise share the same `repsText` and `intensity`. Toggle **Personalizar Series** on any exercise to switch into per-set mode — each set gets its own `targetReps` and `targetRpe` fields.
